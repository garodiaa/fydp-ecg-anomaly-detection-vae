...
### 6.1 What ST-VAE Detects

**Time-Domain Abnormalities:**
- ✅ Myocardial Infarction (ST elevation/depression)
- ✅ Ventricular Hypertrophy (increased R-wave amplitude)
- ✅ Bundle Branch Blocks (widened QRS)
- ✅ T-wave inversions
- ✅ Premature beats (PVCs, PACs)

**Frequency-Domain Abnormalities:**
- ✅ Atrial Fibrillation (irregular RR, no P-wave periodicity)
- ✅ Tachycardia/Bradycardia (shifted heart rate frequency)
- ✅ Heart Rate Variability changes (power spectrum distribution)
- ✅ Arrhythmias (missing periodic components)

**Combined Detection (Both Domains):**
- ✅ **Best overall:** Abnormalities with both morphological and rhythmic features
- Example: Atrial Flutter (sawtooth P-waves in time + 250-350 Hz component in frequency)

### 6.2 Latent Space Analysis

**32-Dimensional Latent Code:**
- Learned representation fusing time + frequency features
- Each dimension captures abstract ECG properties
- Not explicitly interpretable (unlike attention)

**Potential Visualization:**
- t-SNE/UMAP clustering: Do normal/abnormal ECGs separate in latent space?
- Latent arithmetic: Can we interpolate between normal and abnormal?
- Dimension activation: Which latent dims correlate with specific abnormalities?

### 6.3 Reconstruction Error Patterns

**Normal ECG:**
- Low time-domain error (< 0.25)
- Low frequency-domain error (< 100)
- Smooth, accurate reconstruction

**Abnormal ECG:**
- High time-domain error (> 0.40) if morphology abnormal
- High frequency-domain error (> 150) if rhythm abnormal
- Model struggles with novel patterns

**Example Cases:**

| Abnormality | Time Error | Freq Error | Combined | Detected? |
|-------------|------------|------------|----------|-----------|
| Normal Sinus | 0.20 | 70 | Low | ✅ Normal |
| ST-Elevation MI | **0.55** | 95 | High | ✅ Abnormal |
| Atrial Fib | 0.30 | **180** | High | ✅ Abnormal |
| Both | **0.60** | **200** | Very High | ✅ Abnormal |
| Subtle T-inv | 0.28 | 85 | Borderline | ⚠️ Maybe Missed |
...
**Latent Dimension:**
- **Original:** 64 → KL explosion, F1 = 0.066
- **Improved:** 32 → Stable, F1 = 0.90
- **Why 32?** Balance between expressiveness and regularization

**Beta (KL Weight):**
- **Original:** 0.3 → Weak regularization, overfitting
- **Improved:** 0.5 → Strong regularization, generalization
- **Impact:** Prevents model from ignoring latent space

**Frequency Weight:**
- **Original:** 0.5 → Conflict with time loss
- **Improved:** 0.3 (learnable) → Balanced, adapts to ~0.28
- **Impact:** Proper fusion of dual domains

**Dropout:**
- **Original:** 0.1 → Slight overfitting
- **Improved:** 0.2 → Better generalization
- **Applied:** After each ResNet block and in MLPs

### 7.2 Tuning Recommendations

| Hyperparameter | Default | Range | When to Tune |
|----------------|---------|-------|--------------|
| `latent_dim` | 32 | 16-64 | If KL explodes (→reduce) or MSE high (→increase) |
| `beta` | 0.5 | 0.1-1.0 | If collapse (→reduce) or overfit (→increase) |
| `freq_weight` | 0.3 | 0.1-0.5 | If frequency loss dominates (→reduce) |
| `dropout` | 0.2 | 0.1-0.3 | If overfit (→increase) |
| `lr` | 5e-4 | 1e-4-1e-3 | Standard tuning |
...
**1. Model Size (8.2M parameters):**
- Larger than BA-VAE (0.6M) or CAE (0.25M)
- May not fit on very resource-constrained devices
- Mitigated by fast inference (0.11 ms)

**2. FFT Overhead:**
- Requires FFT computation (minimal but present)
- ~0.01-0.02 ms per sample
- Negligible compared to model forward pass

**3. False Positives (14.5%):**
- Higher than BA-VAE (5%)
- Trade-off for high recall
- Acceptable for screening applications

**4. Latent Space Not Interpretable:**
- No attention weights (unlike VAE-BiLSTM-Attn)
- Harder to explain "why" flagged as abnormal
- Could add attention module for interpretability

### 8.2 Future Improvements

**Short-Term:**
1. **Attention Visualization:**
   - Add lightweight attention on top of latent space
   - Show which frequency bands drive detection

2. **Uncertainty Quantification:**
   - Use x_logvar for confidence estimates
   - Flag low-confidence predictions for manual review

3. **Multi-Threshold Strategy:**
   - High precision mode (current)
   - High recall mode (lower threshold)
   - User-selectable operating point

**Long-Term:**
1. **Cross-Database Validation:**
   - Test on MIT-BIH, PhysioNet datasets
   - Domain adaptation techniques

2. **Multi-Label Classification:**
   - Not just normal/abnormal
   - Specific types: MI, arrhythmia, hypertrophy, etc.
   - Multi-task learning
...
### 9.3 Expected Training Time

| Hardware | Batch Size | Time per Epoch | Total (50 epochs) |
|----------|-----------|----------------|-------------------|
| RTX 3060 | 64 | ~50 sec | **~42 min** |
| RTX 3090 | 64 | ~30 sec | **~25 min** |
| CPU (16 cores) | 64 | ~8 min | **~6.7 hours** |
...
**Spectral-Temporal VAE (ST-VAE)** is the **best-performing model** for ECG anomaly detection:

✅ **Highest F1-Score (0.90):** Best overall performance  
✅ **Highest Recall (0.91):** Catches 91% of abnormalities  
✅ **Excellent AUC-ROC (0.93):** Strong discrimination  
✅ **Fastest VAE Inference (0.11 ms):** Real-time capable  
✅ **Dual-Domain Learning:** Time + Frequency representations  
✅ **ResNet Backbone:** Better gradient flow, deeper networks  

**Key Innovation:** Parallel time-frequency encoding with adaptive fusion.

**Best For:**
- High-recall anomaly screening
- Balanced precision-recall applications
- Real-time ECG monitoring
- Research benchmarking

**Trade-offs:**
- Larger model (8.2M params) vs BA-VAE (0.6M)
- Slightly more false positives (14.5%) vs BA-VAE (5%)
- Less interpretable than attention models

**Recommendation:** **Use ST-VAE as primary model** for ECG anomaly detection. Best overall performance with acceptable trade-offs.

---

*Document Version: 1.0*  
*Last Updated: December 1, 2025*  
*Model Version: ST-VAE (Improved Architecture)*
