...
def main():
    p = argparse.ArgumentParser(description='Train ECG anomaly detection models')
    p.add_argument('--model', type=str, choices=['vae_bilstm_attn', 'vae_gru', 'vae_bilstm_mha', 'hlvae', 'cae'], required=True, help='Model to train')
    p.add_argument('--dataset', type=str, choices=['ptbxl', 'mimic', 'both'], default='ptbxl', help='Dataset to use')
    ...
    p.add_argument('--window_size', type=int, default=500, help='Window size')
    p.add_argument('--stride', type=int, default=125, help='Stride for windows')
    ...
    if args.dataset == 'ptbxl':
        full_ds = WindowDataset(ptbxl_dir=str(DATA_DIR / "ptbxl"), dataset='ptbxl', window_size=args.window_size, stride=args.stride, sample_length=5000)
    elif args.dataset == 'mimic':
        full_ds = WindowDataset(mimic_dir=str(DATA_DIR / "mimic"), dataset='mimic', window_size=args.window_size, stride=args.stride, sample_length=5000)
    elif args.dataset == 'both':
        full_ds = WindowDataset(ptbxl_dir=str(DATA_DIR / "ptbxl"), mimic_dir=str(DATA_DIR / "mimic"), dataset='both', window_size=args.window_size, stride=args.stride, sample_length=5000)
    ...
    if args.model == 'vae_bilstm_attn':
        ...
        model = VAE_BILSTM_MHA(seq_len=args.window_size, n_leads=12, latent_dim=args.latent_dim, beta=args.beta).to(device)
    elif args.model == 'hlvae':
        model = HLVAE(n_leads=12, seq_len=args.window_size, global_latent_dim=args.latent_dim, local_latent_dim=args.latent_dim//2, beta_g=args.beta, beta_l=args.beta).to(device)
    elif args.model == 'cae':
