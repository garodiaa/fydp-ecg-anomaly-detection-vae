...
# Global tqdm configuration you can tweak
TQDM_CONFIG = {
    "dynamic_ncols": True,
    "leave": False,
    "ascii": True,      # IMPORTANT for Windows / VSCode terminals
    "mininterval": 0.1,
}
...
def train_epoch(model, dataloader, optimizer, device, args, desc="Train"):
    model.train()
    total_loss = 0.0
    total_recon = 0.0
    total_kl = 0.0
...
def validate_epoch(model, dataloader, device, args, desc="Validate"):
    model.eval()
    total_loss = 0.0
    total_recon = 0.0
    total_kl = 0.0
...
def main():
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # TRAIN (one single-line progress bar)
        train_loss, train_recon, train_kl = train_epoch(model, train_loader, optimizer, device, args, desc=f"Epoch {epoch:03d} Train")

        # VALIDATE (one single-line progress bar)
        val_loss, val_recon, val_kl = validate_epoch(model, val_loader, device, args, desc=f"Epoch {epoch:03d} Validate")
        ...
        if args.model == 'vae':
            ...
            # Use tqdm.write so we don't corrupt the progress bar output
            tqdm.write(f"Epoch {epoch:03d} per-dim posterior variance: {post_var.tolist()}")
            tqdm.write(f"Epoch {epoch:03d} per-dim posterior mean: {mean_mu.tolist()}")
            tqdm.write(f"Epoch {epoch:03d} per-dim KL: {kl_per_dim.tolist()}")
        else:
            low_kl_dims = 0
            total_dims = 0
        ...
        # Print a clean epoch summary below the progress bars
        if args.model == 'vae':
            tqdm.write(
                (f"Epoch {epoch:03d} → "
        ...
        # GPU Memory Status (also via tqdm.write so it doesn't break bars)
        if torch.cuda.is_available():
