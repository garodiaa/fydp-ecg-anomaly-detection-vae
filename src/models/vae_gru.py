...
class VAE(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        attn_out, attn_weights = self.attn(h_seq, h_seq, h_seq, need_weights=True)
        h_seq = attn_out  
        ...
        lead_w = torch.ones(B, T, self.n_feats, device=x.device) / self.n_feats

        return x_recon, x_logvar, mu, logvar, attn_weights, lead_w
