...
# Script path: <repo_root>/src/data_processing/pre-process.py
# We want PROJECT_ROOT to point to the repo root (two levels up from script dir)
...
def process_ptbxl(input_dir: Path, csv_path: Path, output_dir: Path, resolution: str = 'auto'):
    """
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc='PTB-XL'):
        # Determine which filename to use based on requested resolution or availability
        chosen_name = None
        hr_name = str(row.get('filename_hr', ''))
        lr_name = str(row.get('filename_lr', ''))
        ...
        if resolution == 'hr':
            ...
            # Prefer HR; fallback to LR if HR record header file (.hea) exists
            hr_path = input_dir / f"{hr_name}.hea"
            if hr_name and hr_path.exists():
                chosen_name = hr_name
            else:
                chosen_name = lr_name

        rec_path = input_dir / chosen_name
        rec_file = Path(chosen_name).name  # e.g. "21837_hr" or "21837_lr"
        try:
            # Check if .hea file exists (wfdb.rdrecord expects base path without extension)
            hea_file = Path(str(rec_path) + '.hea')
            if not hea_file.exists():
                raise FileNotFoundError(f"Record header not found: {hea_file}")
            ...
            print(f"⚠️  PTB-XL {chosen_name} failed: {e}")
...
def main():
    ...
    p.add_argument('--resolution', choices=['auto', 'hr', 'lr'], default='auto',
                   help='PTB-XL only: select record resolution (hr, lr) or auto fallback (default).')
    args = p.parse_args()
    ...
    if args.dataset == 'ptbxl':
        print(f'Starting PTB-XL preprocessing (resolution={args.resolution})...')
        process_ptbxl(
